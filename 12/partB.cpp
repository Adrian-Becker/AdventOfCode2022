...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < height_map.size(); ++i) {
        ...
        for (int j = 0; j < current.size(); ++j) {
            if (current[j] == 0) {
                ...
                if (best_path.empty() || (path.size() < best_path.size() && !path.empty())) {
    ...
    for (int k = animate ? 0 : ((int) best_path.size() - 1); k < best_path.size(); ++k) {
        ...
        for (int i = 0; i < height_map.size(); ++i) {
            ...
            for (int j = 0; j < current.size(); ++j) {
                ...
                if (best_start.x == i && best_start.y == j) {
                ...
                } else if (std::find(best_path.begin() + (best_path.size() - k), best_path.end(), Position{i, j}) !=
                           best_path.end()) {
    ...
    std::cout << "Paths tested: \t\t" << tested << std::endl;
    std::cout << "Steps required: \t" << best_path.size() - 1 << std::endl;
}