...
struct Grid {
    ...
    void display(int step, std::set<Position> &current, std::set<Position> &history) {
...
int find_path(Grid &grid, bool animate) {
    ...
    while (true) {
        ...
        for (const Position &position: current) {
            ...
            if (position.x == grid.width - 1 && position.y == grid.height - 1) {
                return steps;
            }
        ...
        if (animate) {
            grid.display(steps, current, history);
...
int main(int argc, char *argv[]) {
    ...
    int size = find_path(grid, animate);


    if (animate) std::cout << std::endl;
    std::cout << "Size of field: " << width << "x" << height << std::endl;
    std::cout << "Required number of steps: " << size << std::endl;
}
